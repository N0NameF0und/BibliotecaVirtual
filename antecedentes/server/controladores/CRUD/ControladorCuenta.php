...
class ControladorCuenta extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO Cuenta (idCuenta,idUsuario,contraseña) VALUES (?,?,?);";
      $parametros = array($cuenta->idCuenta,$cuenta->idUsuario,$cuenta->contraseña);
   ...
   {
      $parametros = array($cuenta->idCuenta,$cuenta->idUsuario,$cuenta->contraseña,$cuenta->id);
      $sql = "UPDATE Cuenta SET idCuenta = ?,idUsuario = ?,contraseña = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
